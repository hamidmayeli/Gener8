...
internal static class SourceProducer
{
    ...
    {
        EmitModel(context, target, nullableEnabled);
        EmitExtensions(context, target, nullableEnabled);
        if (target.Repository != RepositoryKind.None)
            EmitRepository(context, target, nullableEnabled);
    }

    private static void EmitModel(SourceProductionContext context, TargetClass target, bool nullableEnabled)
    {
