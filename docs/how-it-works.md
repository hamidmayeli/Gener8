...
Takes a `ClassTarget` and writes a `StringBuilder`-based C# source string:
...
The hint name for the generated file is `{Namespace}.{ClassName}.g.cs` (or `{ClassName}.g.cs` for the global namespace). Roslyn uses the hint name to identify the source in diagnostics and persisted output.
