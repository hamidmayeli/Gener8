...
| `Repository.g.cs` | `Gener8.Repository<T>` abstract base class |
...
**`EmitRepository`** (only when `target.Repository != RepositoryKind.None`) writes a concrete repository scaffold (`{ClassName}{DynamoDb|MongoDb}Repository.g.cs`):
...
[namespace X {]
  {accessibility} class {ClassName}{DynamoDb|MongoDb}Repository : global::Gener8.Repository<{ClassName}>
  {
      // DynamoDb: private readonly IAmazonDynamoDB _client + DynamoDbRepositorySettings _settings
      // MongoDb:  private readonly IMongoCollection<{ClassName}> _collection (resolved in ctor)

      public override Task<{ClassName}?> GetByIdAsync(...)      => throw new NotImplementedException();
      public override Task<IEnumerable<{ClassName}>> GetAllAsync(...) => throw new NotImplementedException();
      public override Task SaveAsync(...)                        => throw new NotImplementedException();
      public override Task DeleteAsync(...)                     => throw new NotImplementedException();
  }
[}]
```

The hint name for each file is `{Namespace}.{ClassName}[Extensions|DynamoDbRepository|MongoDbRepository].g.cs` (or without namespace prefix for the global namespace).
