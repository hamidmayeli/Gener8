...
The converter classes are provided by `Gener8.Extensions.DynamoDB`.

Requires the `Gener8.Extensions.DynamoDB` and `AWSSDK.DynamoDBv2` NuGet packages.
...
internal partial class ProductRepository : Gener8.MongoDbRepository<Product, ProductDto>
{
    public ProductRepository(IMongoDbRepositoryContext context) : base(context, "ProductDto") {}
    protected override Product    ToModel(ProductDto dto)   => dto.ToModel();
    protected override ProductDto ToDto  (Product    model) => model.ToDto();
}
```
...
internal partial class ProductRepository : Gener8.RepositoryBase<Product, ProductDto>
{
...
The abstract base classes (`DynamoDbRepository<TModel, TDto>` and `MongoDbRepository<TModel, TDto>`) come from `Gener8.Extensions.DynamoDB` and `Gener8.Extensions.MongoDB`. The context interfaces (`IDynamoDbRepositoryContext` and `IMongoDbRepositoryContext`) are also defined in those extension packages.

The `IRepository<TModel>` interface (from `Gener8.Abstractions`, bundled via `Gener8`) defines the standard CRUD contract:
...
- **GEN005** is raised for each name in `OnlyInclude` that does not match any public property on the model it applies to. This covers dotted sub-paths too: `"Customer.Bogus"` is reported against the nested `Customer` model.
